...
async function loader({
  request,
  params: { matricula: matriculaAsString },
  context,
}) {
  ...
  const tarjetas = await context.api.getTarjetas(matricula, todas);
  console.timeEnd("tarjetas" + matricula);
  console.time("historico" + matricula);
  const historico = null; // await getHistorico(matricula, monthsParam, context);
  ...
  const player = {}; //await getPlayer(matricula, context);
...
function Tarjetas() {
  ...
  const untilDate = handicapDate ? date.make7Am(new Date(handicapDate)) : null;
