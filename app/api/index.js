...
export function createApi(env, ctx) {
  const { handicap, jugadores } = makeDb(env);
  ...
  async function findPlayers(searchString) {
    ...
    ctx.waitUntil(saveHistorico(playersInfo)); // save but no need to wait on response
    return playersInfo;
  }
      ...
      if (all9Holes[i + 1]) {
        selected9Holes = [
          ...selected9Holes,
          {
            ...
            is9: true,
            diferencial:
              all9Holes[i].diferencial + all9Holes[i + 1].diferencial,
          },
        ];
      }
    }
  ...
  return {
    findPlayers,
    getHistorico: handicap.getHistorico,
    getPlayer,
    getTarjetas,
    saveHistorico,
  };
