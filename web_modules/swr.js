import { g as react_10, h as react_19, b as react_26, f as react_27, j as react_18, l as react_24, i as react_21, m as react_23, p as react } from './common/index-631f69c8.js';
...
const SWRConfigContext = react_10({});
...
const useIsomorphicLayoutEffect = IS_SERVER ? react_21 : react_23;
...
function useSWR(...args) {
    ...
    config = Object.assign({}, defaultConfig, react_19(SWRConfigContext), config);
    ...
    const stateDependencies = react_26({
        data: false,
        error: false,
        isValidating: false
    });
    const stateRef = react_26({
        data: initialData,
        error: initialError,
        isValidating: false
    });
    const rerender = react_27(null)[1];
    let dispatch = react_18(payload => {
    ...
    const unmountedRef = react_26(false);
    const keyRef = react_26(key);
    const boundMutate = react_18((data, shouldRevalidate) => {
        return mutate(key, data, shouldRevalidate);
    }, [key]);
    // start a revalidation
    const revalidate = react_18(async (revalidateOpts = {}) => {
        if (!key || !fn)
            return false;
...
function useSWRPages(pageKey, pageFn, SWRToOffset, deps = []) {
    ...
    const [pageCount, setPageCount] = react_27(cache.get(pageCountKey) || 1);
    const [pageOffsets, setPageOffsets] = react_27(cache.get(pageOffsetKey) || [null]);
    const [pageSWRs, setPageSWRs] = react_27([]);
    const pageFnRef = react_26(pageFn);
    const emptyPageRef = react_26(false);
    // Page component (wraps `pageFn`)
    // for performance reason we need to memorize it
    const Page = react_18(props => {
    ...
    const _pageFn = react_18(pageFn, deps);
    pageFnRef.current = _pageFn;
    const pages = react_24(() => {
